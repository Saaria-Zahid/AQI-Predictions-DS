...
def generate_forecast(hours: int = FORECAST_HOURS) -> pd.DataFrame:
    ...
    history = pd.DataFrame(
        list(feature_store.find({}).sort('datetime', -1).limit(200))
    )
    ...
    last_hist_time = history['datetime'].iloc[-1]
    now_hour = pd.Timestamp.now().floor('h')

    if last_hist_time >= now_hour:
        current_time = last_hist_time
    else:
        current_time = now_hour - timedelta(hours=1)
    ...
    for step in range(hours):
        ...
        # recompute only last 50 rows for speed
        history_tail = recompute_features(history.tail(60))
        history.update(history_tail)
